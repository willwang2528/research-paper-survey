...
- 按任务选渠道，不要求每次全部搜索；近期工作可浏览目标会议最近几届目录并查看潜在相关摘要，补查标题没有使用预设关键词的论文，记录实际覆盖范围。
...
Skill 入口：[SKILL.md](skills/research-paper-survey/SKILL.md)。逐篇介绍统一以英文题名、中文题名、日期/收录行、独立作者单位行（中国机构中文、其他机构英文）、无前缀最新版 arXiv 链接和摘要总结开头，详见 [展示模板](skills/research-paper-survey/references/templates.md)。方法依据、协议模板、数据契约、迭代协议按需加载。
